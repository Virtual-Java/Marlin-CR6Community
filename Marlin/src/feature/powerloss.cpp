...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=POWER_LOSS_ZRAISE*/, const bool raised/*=false*/) {
  ...
  if (force
    ...
    void PrintJobRecovery::outage() {
      ...
      static float volts_last = 0;

      if(!enabled) {
        return ;
      }

      if(power_monitor.getVolts() < 23.00) {

        SERIAL_ECHOLNPAIR("v:", power_monitor.getVolts());

        if(cnt >= 3) {
            _outage();
        }

        if(power_monitor.getVolts() < volts_last) {
            cnt++;
        }
      ...
      volts_last = power_monitor.getVolts();
    }
#endif
